...
export default {
  data() {
	...
	youxiang : false,
      ...
      rules: {
          zhigongzhanghao: [
                { required: true, message: '职工账号不能为空', trigger: 'blur' },
          ],
          mima: [
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `susheguanli/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.susheguanliCrossAddOrUpdateFlag = false;
						 this.parent.search();
