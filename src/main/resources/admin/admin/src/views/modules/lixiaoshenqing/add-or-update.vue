...
export default {
  ...
  methods: {
	...
	this.ruleForm.shenqingshuoming = this.ruleForm.shenqingshuoming.replace(reg,'../../../springbootttxcu/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `lixiaoshenqing/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.lixiaoshenqingCrossAddOrUpdateFlag = false;
						 this.parent.search();
